...
pageElements = {
  ...
  content: {
    ...
    main: {
      ...
      creditsParent: document.getElementById("creditsHtmlParent"),
    },
  },
};
...
// Import Credits Json
let credits = {};
function credit2html(original) {
  let html = "";
  Array.from(original).forEach((s) => {
    if (s.section != "Mojang Studio") {
      s.disciplines.forEach((d) => {
        html += `<s-card class="main-card"><h2>${s.section} - ${d.discipline}</h2>`;
        d.titles.forEach((t) => {
          html += `<h3>${t.title}</h3><p>`;
          t.names.forEach((n) => {
            html += `${n}<br>`;
          });
          html += `</p>`;
...
fetch("https://kdxiaoyi.top/Streack/resourcepack/assets/minecraft/texts/credits.json").then(response => {
  if (!response.ok) {
  ...
  credits = data;
  pageElements.content.main.creditsParent.innerHTML = credit2html(credits);
}).catch(err => {
  console.error(err);
  msg("加载配置文件时发生错误，检查你的网络连接后重试。", "好", true);
  pageElements.content.main.creditsParent.innerHTML = `<s-card class="main-card"><div slot="headline">发生了一点点错误……</div> <div slot="text">无法连接到服务器获取资源文件，检查网络连接后再试。</div></s-card>`;
});
